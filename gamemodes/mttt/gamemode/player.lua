...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
   ...
   if GetRoundState() == ROUND_ACTIVE then
      ...
      if IsValid(attacker) and attacker:IsPlayer() then
         ...
         DamageLog(Format("KILL:\t %s [%s] killed %s [%s] with %s", attacker:Nick(), attacker:GetRoleString(), ply:Nick(), ply:GetRoleString(),dmginfo:GetInflictor()))
      else
...
function GM:PlayerTakeDamage(ent, infl, att, amount, dmginfo)
   ...
   if ent != att and IsValid(att) and att:IsPlayer() and GetRoundState() == ROUND_ACTIVE and math.floor(dmginfo:GetDamage()) > 0 then
      ...
      DamageLog(Format("DMG: \t %s [%s] damaged %s [%s] for %d dmg with %s", att:Nick(), att:GetRoleString(), ent:Nick(), ent:GetRoleString(), math.Round(dmginfo:GetDamage()),dmginfo:GetInflictor()))
   end
end
