...
desc = "Spin the chamber and pull the trigger"
};
...
if SERVER then
  ...
  timer.Simple(3,function()
    print(chamberPosition)
    if(chamberPosition >= 6) then
      sound.Play(self.Primary.Sound, ply:GetPos())
      ply:TakeDamage(500, ply, gun)
      chamberPosition = 0
    else
      sound.Play("Weapon_Pistol.Empty", ply:GetPos())
      chamberPosition = chamberPosition +1
    end
    inUse = false
  end)
end
...
function SWEP:SecondaryAttack()
  self:SetNextPrimaryFire( CurTime() + self.Primary.Delay )
  self:SetNextPrimaryFire( CurTime() + self.Primary.Delay )
  chamberPosition = math.random(1,6)
